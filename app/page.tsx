...
export default function TVTracker() {
  ...
  const addShowFromSearch = (searchResult: ShowSearchResult, metadata?: ShowMetadata) => {
    const now = new Date().toISOString()
  ...
  const deleteShow = (showId: number) => {
    setShows(shows.filter((show) => show.id !== showId))
  }

  const updateSeasonStatus = (showId: number, seasonId: number, newStatus: WatchStatus) => {
    setShows(
      shows.map((show) => {
        if (show.id === showId) {
          return {
